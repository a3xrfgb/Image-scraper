...
search_keyword = input("Enter the search keyword to scrape from Alamy Images: ").strip()
...
# Build URL for Alamy
search_URL = f"https://www.alamy.com/search.html?qt={search_keyword}"
...
try:
    while True:  # Run indefinitely until user stops
        try:
            ...
            thumbnails = driver.find_elements(By.CSS_SELECTOR, "img.lazyload")
            
            if not thumbnails:
                thumbnails = driver.find_elements(By.CSS_SELECTOR, "img[src^='https://c.alamy.com/thumbs']")
            ...
            try:
                ...
                link_element = thumbnail.find_element(By.XPATH, "..")
            ...
            # Method 1: Try to find the full-size image on the detail page
            try:
                full_image = driver.find_element(By.CSS_SELECTOR, "img#previewImage")
                src = full_image.get_attribute("src")
                
                if src and src.startswith("http") and "alamy.com" in src:
            ...
            try:
                ...
                for img in images:
                    try:
                        ...
                        # Filter out thumbnails and non-relevant images
                        if (src and src.startswith("http") and 
                            "alamy.com" in src and 
                            "thumbs" not in src and
                            "logo" not in src):
