...
import time
import requests
from PIL import Image  # Added for checking image resolution
from io import BytesIO  # Added for loading image from bytes
...
# Define minimum resolution for hi-res (e.g., 1024x768 or higher)
MIN_WIDTH = 1024
MIN_HEIGHT = 768
            ...
            if 'image' in content_type:
                ...
                try:
                    ...
                    if width >= MIN_WIDTH and height >= MIN_HEIGHT:
                        file_path = os.path.join(folder_name, f"{num}.jpg")
                        with open(file_path, "wb") as file:
                            file.write(response.content)
                        downloaded_urls.add(url)  # Mark as downloaded
                        print(f"Downloaded hi-res image {num} ({width}x{height})")
                        return True
                    else:
                        print(f"Skipping low-res image {num} ({width}x{height})")
                        return False
...
# Build URL - Strictly using the user-provided keyword without modifications
